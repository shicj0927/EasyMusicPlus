...
class LibraryManager:
    ...
    def get_media_by_index(self, songlistId: str, index: int) -> MediaItem_V10:
        if not self.loaded:
            raise RuntimeError("Library not loaded")
        return self.repository.get_media(
            self.repository.get_playlist(songlistId).media_ids[index]
        )
    ...
    def new_playlist(self, title: str):
        if not self.loaded:
            raise RuntimeError("Library not loaded")
        new_playlist = PlayList_V20(
            id=self.gen_new_playlist_id(), title=title, media_ids=[]
        )
