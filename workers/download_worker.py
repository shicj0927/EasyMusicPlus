        ...
        try:
            ...
            if songData == None or songData == {}:
                ...
                if url == None or url == "":
                    if mediaId == None or mediaId == "":
                        self.downloadFinishedSignal.emit(
                            {"status": "error", "message": "下载失败：缺少必要参数"}
                        )
                        return
                    else:
                        songData["id"] = mediaId
                    ...
                    if "bilibili.com" in urlData.netloc:
                        ...
                        self.downloadFinishedSignal.emit(
                            {"status": "error", "message": "下载失败：不支持的URL"}
                        )
                        return
                self.downloadLogSingnal.emit(
                    f"提取到媒体ID：{songData['id']}，来源：{songData['source']}"
                )
                # 请求title和artist等信息
                if songData["source"] == "bilibili":
                    ...
                    songData["artists"] = [
                        ar["name"] for ar in dat["songs"][0]["artists"]
                    ]
                self.downloadLogSingnal.emit(
                    f"获取到歌曲名称：{songData['title']} - {', '.join(songData['artists'])}"
                )
            else:
                # 为songData补全url等字段
                if songData["source"] == "bilibili":
                ...
                tmp = {
                    ...
                    "title": songData.get("name", ""),
                    "artists": songData.get("artist", []),
                    "url": songData["url"],
                }
                songData = tmp
                self.downloadLogSingnal.emit(
                    f"使用提供的歌曲信息，名称：{songData['title']} - {', '.join(songData['artists'])}"
                )
            ...
            path = (
                os.path.join(
                    path,
                    safe_file_name(
                        f"{songData['title']} - {', '.join(songData['artists'])}"
                    ),
                )
                + randomId
            )
            ...
            try:
                ...
                if response.status_code == 200:
                    coverPath = os.path.join(
                        path,
                        safe_file_name(
                            f"{songData['title']} - {', '.join(songData['artists'])}.jpg"
                        ),
                    )
            ...
            if songData["source"] == "bilibili":
                ...
                ydl_opts = {
                    ...
                    "outtmpl": os.path.join(
                        path,
                        safe_file_name(
                            f"{songData['title']} - {', '.join(songData['artists'])}.%(ext)s"
                        ),
                    ),
                    "noplaylist": True,
                    "http_headers": {
                ...
                try:
                    ...
                    songData["video_path"] = os.path.join(
                        path,
                        safe_file_name(
                            f"{songData['title']} - {', '.join(songData['artists'])}.mp4"
                        ),
                    )
                    self.downloadFinishedSignal.emit(
                        {
                            "status": "success",
                            "message": f"下载完成，保存路径：{path}",
                            "result": songData,
                        }
                    )
                except Exception as e:
                    self.downloadFinishedSignal.emit(
                        {"status": "error", "message": f"下载失败：{str(e)}"}
                    )
            elif songData["source"] == "netease":
                # 下载歌词
                ...
                if json:
                    ...
                    if "lrc" in lyricData and "lyric" in lyricData["lrc"]:
                        ...
                        lyricPath = os.path.join(
                            path,
                            safe_file_name(
                                f"{songData['title']} - {', '.join(songData['artists'])}.lrc"
                            ),
                        )
                        with open(lyricPath, "w", encoding="utf-8") as f:
                            f.write(lyricContent)
                        self.downloadLogSingnal.emit(
                            f"歌词下载完成，保存路径：{lyricPath}"
                        )
                        songData["lyric_path"] = lyricPath
                    else:
                        self.downloadLogSingnal.emit("未找到歌词信息")
                ...
                fileName = safe_file_name(
                    f"{songData['title']} - {', '.join(songData['artists'])}.mp3"
                )
                filePath = os.path.join(path, fileName)
                try:
                    with requests.get(
                        downloadUrl, headers=headers, stream=True, timeout=10
                    ) as r:
                        r.raise_for_status()
                        ...
                        with open(filePath, "wb") as f:
                            for chunk in r.iter_content(chunk_size=8192):
                                if chunk:
                                    ...
                                    progress = int(
                                        downloaded_length / total_length * 100
                                    )
                                    self.procressUpdateSignal.emit(progress)
