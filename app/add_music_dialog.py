...
class AddMusicDialog(QDialog):
    ...
    def __init__(
        self, parent, libraryManager: LibraryManager, songlist_id: str, dlconfig: dict
    ):
    ...
    def bind_signals(self):
        ...
        self.ui.qPushButton_netease_songlist.clicked.connect(
            self.open_get_songlist_dialog
        )
        self.ui.qPushButton_netease_list_download.clicked.connect(
            self.open_netease_list_download_dialog
        )
    ...
    def add_from_data(self):
        ...
        if media_id:
            self.libraryManager.add_media_to_playlist(
                self.songlist_id, self.libraryManager.get_media_by_id(media_id)
            )
    ...
    def add_from_list(self):
        ...
        self.addFromListDialog = AddMusicFromListDialog(
            self.libraryManager, self.songlist_id
        )
    ...
    def open_downloader(self):
        if self.dlconfig.get("downloadPath"):
            downloadDialog = DownloadDialog(parent=self.parent, config=self.dlconfig)
            downloadDialog.downloadCompletedSignal.connect(
    ...
    def open_get_songlist_dialog(self):
        self.getDialog = GetNeteaseSonglistDialog(
            self.libraryManager, self.songlist_id, self.dlconfig
        )
