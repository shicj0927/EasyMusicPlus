...
class GetNeteaseSonglistDialog(QDialog):
    ...
    def start_download(self):
        ...
        self.next_index = 0
        self.error_indexs = []
        self.ui.qPlainTextEdit_log.appendPlainText(
            f"开始下载 {len(self.songlist)} 首歌曲..."
        )
        self.download_next()
    ...
    def download_next(self):
        ...
        for i in all_songs.media_ids:
            ...
            if (
                song.source == "netease"
                and song.source_id == self.songlist[self.next_index]
            ):
                song_id = i
        if song_id != "":
            self.ui.qPlainTextEdit_log.appendPlainText(
                f"{self.next_index + 1}: 本地已有存档"
            )
            ...
            self.download_dialog.downloadCompletedSignal.connect(
    ...
    def on_song_download_completed(self, download_result):
        if "error" in download_result:
            self.ui.qPlainTextEdit_log.appendPlainText(
                f"{self.next_index + 1}: 下载失败"
            )
            self.error_indexs.append(self.next_index)
        else:
            self.ui.qPlainTextEdit_log.appendPlainText(
                f"{self.next_index + 1}: 下载成功"
            )
    ...
    def update_progress(self):
        if len(self.songlist):
            self.ui.qProgressBar_progress.setValue(
                int(100 * self.next_index / len(self.songlist))
            )
