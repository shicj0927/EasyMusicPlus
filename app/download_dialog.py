...
class DownloadDialog(QDialog):
    ...
    def __init__(self, parent=None, config=None, autoStartId=None):
        ...
        self.downloadWorker.procressUpdateSignal.connect(
            self.ui.qProgressBar_download.setValue
        )
    ...
    def init_ui(self):
        ...
        self.ui.qLineEdit_outputPath.setPlaceholderText(
            "输入下载路径，默认为当前目录下downloads文件夹"
        )
    ...
    def bind_signals(self):
        ...
        self.ui.qPushButton_downloadId.clicked.connect(
            self.on_download_with_mediaId_clicked
        )
        self.ui.qPushButton_downloadUrl.clicked.connect(
            self.on_download_with_url_clicked
        )
    ...
    def on_search_finished(self, result):
        ...
        for row, i in enumerate(result):
            ...
            model.appendRow([name, artist, download_item])
            btn = QPushButton("下载")
            btn.clicked.connect(lambda _, r=row: self.on_download_with_info_clicked(r))
    ...
    def on_download_with_info_clicked(self, row):
        ...
        self.start_download(
            songInfo, None, None, self.ui.qCheckBox_randomIdFlag.isChecked()
        )
    ...
    def on_download_with_mediaId_clicked(self):
        ...
        self.ui.qTabWidget_mainTabs.setCurrentIndex(1)
        self.start_download(
            None, None, mediaId, self.ui.qCheckBox_randomIdFlag.isChecked()
        )
    ...
    def start_download(self, songInfo, url, mediaId, randomIdFlag=False):
        ...
        self.ui.qTextBrowser_taskDetail.append(
            f"下载参数：{songInfo}, {url}, {mediaId}, {randomIdFlag}"
        )
        downloadPath = self.ui.qLineEdit_outputPath.text()
        if downloadPath == "":
            downloadPath = "./downloads"
        self.startDownloadSignal.emit(
            songInfo, url, mediaId, downloadPath, randomIdFlag
        )
