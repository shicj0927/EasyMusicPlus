...
class NeteaseListDownloadDialog(QDialog):
    ...
    def __init__(
        self, libraryManager: LibraryManager, songlist_id: str, dlconfig: dict
    ):
        super().__init__()
    ...
    def start_workers(self):
        ...
        self.getNeteaseSonglistWorker.getNeteaseSonglistFinishedSignal.connect(
            self.on_get_finished
        )
    ...
    def on_get_finished(self, result):
        if result == None:
            ...
            for row in range(len(self.result.get("tracks"))):
                ...
                model.appendRow([id, name, artist, status_item, progress_item])
                psb = QProgressBar()
    ...
    def from_library_to_list(self, index):
        self.libraryManager.add_media_to_playlist(
            self.songlist_id,
            self.libraryManager.get_media_by_id(self.id_dict[self.id_list[index]]),
        )
    ...
    def start_download(self):
        ...
        self.current_index = 0
        while (
            self.current_index < len(self.is_downloaded)
            and self.is_downloaded[self.current_index]
        ):
            self.from_library_to_list(self.current_index)
            self.current_index += 1
        self.refreshPlaylistSignal.emit()
        if (
            self.current_index == len(self.is_downloaded) - 1
            and self.is_downloaded[self.current_index]
        ):
            self.from_library_to_list(self.current_index)
            self.refreshPlaylistSignal.emit()
        ...
        self.startDownloadSignal.emit(
            None,
            None,
            self.id_list[self.current_index],
            self.dlconfig["downloadPath"],
            True,
        )
    ...
    def on_download_finished(self, result):
        ...
        if result.get("status") != "success":
            ...
            res = download_result_to_media_item(
                result.get("result"), self.libraryManager.gen_new_media_id()
            )
            self.libraryManager.add_media_to_playlist(self.songlist_id, res)
        self.refreshPlaylistSignal.emit()
        self.current_index += 1
        while (
            self.current_index < len(self.id_list)
            and self.is_downloaded[self.current_index]
        ):
            self.from_library_to_list(self.current_index)
            self.current_index += 1
        self.ui.qProgressBar_progress.setValue(
            int(100 * self.current_index / len(self.id_list))
        )
        if (
            self.current_index == len(self.id_list)
            or self.is_downloaded[self.current_index]
        ):
