...
class LibraryRepository:
    ...
    def load_library(self, path: str):
        ...
        if data.get("version") == "1.0":
            ...
            library_V10.play_lists = [
                PlayList_V10.from_dict(pl) for pl in data.get("play_lists", [])
            ]
            session_data = data.get("session")
            library_V10.session = (
                Session_V10.from_dict(session_data) if session_data else None
            )
            self.library = upgrade_library_v10_to_v20(library_V10)
            self.save_library()
        elif data.get("version") == "2.0":
            self.library.play_lists = [
                PlayList_V20.from_dict(pl) for pl in data.get("play_lists", [])
            ]
            media_items = data.get("media_data", {}).get("media_items", [])
            self.library.media_data.media_items = [
                MediaItem_V10.from_dict(m) for m in media_items
            ]
    ...
    def save_library(self):
        ...
        data = {
            ...
            "play_lists": [asdict(pl) for pl in self.library.play_lists],
        }
        print(
            f"Saving library to {os.path.join(self.library.master_folder, 'library.json')}"
        )
    ...
    def insert_into_playlist(self, playlist_id: str, media: MediaItem_V10):
        ...
        for m in self.library.media_data.media_items:
            if (m.id, m.source, m.source_id, m.url) == (
                media.id,
                media.source,
                media.source_id,
                media.url,
        ...
        for pl in self.library.play_lists:
            if pl.id == playlist_id:
                try:
                    ...
                    raise ValueError(
                        f"Media {media_id} not found in playlist {playlist_id}"
                    )
        raise ValueError(f"Playlist {playlist_id} not found")
