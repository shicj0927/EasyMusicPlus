...
def download_result_to_media_item(download_result: dict, id: str) -> MediaItem_V10:
    return MediaItem_V10(
        ...
        audio_track=AudioTrack(path=get_rel_path(download_result["audio_path"]))
        if download_result["audio_path"] is not None
        and os.path.exists(download_result["audio_path"])
        else None,
        video_track=VideoTrack(path=get_rel_path(download_result["video_path"]))
        if download_result["video_path"] is not None
        and os.path.exists(download_result["video_path"])
        else None,
        lyric_track=LyricTrack(path=get_rel_path(download_result["lyric_path"]))
        if download_result["lyric_path"] is not None
        and os.path.exists(download_result["lyric_path"])
        else None,
    )
